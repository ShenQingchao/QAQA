...
class QAData(object):
    ...
    def load_dataset(self, tokenizer, do_return=False):
        ...
        if self.load and os.path.exists(preprocessed_path):
            ...
            self.logger.info ("Start tokenizing {} ...".format(self.data_path))
            ...
            if self.args.do_lowercase:
                questions = [question.lower() + " </s>" for question in questions]
                answers = [answer.lower() + " </s>" for answer in answers]
            if self.args.append_another_bos:
                questions = ["<s> "+question for question in questions]
                answers = ["<s> " +answer for answer in answers]
            self.logger.info("Tokenizing questions...")
            question_input = tokenizer.batch_encode_plus(questions,
                                                         pad_to_max_length=True,
                                                         max_length=self.args.max_input_length,
                                                         truncation=True)
            self.logger.info("Tokenizing answers...")
            ...
            if self.load:
                self.logger.info("Dumping into {} ...".format(preprocessed_path))
    ...
    def load_dataloader(self, do_return=False):
        self.dataloader = MyDataLoader(self.args, self.dataset, self.is_training, no_shuffle=self.args.no_shuffle)
        print ("Dataloader Sampler:", self.dataloader.sampler)
        if do_return:
...
class MyDataLoader(DataLoader):
    ...
    def __init__(self, args, dataset, is_training, no_shuffle):
        if is_training:
            sampler=SequentialSampler(dataset) if no_shuffle else RandomSampler(dataset)
